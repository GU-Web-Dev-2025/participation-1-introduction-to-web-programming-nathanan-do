...
public class Meld {
    ...
    /**
 * Checks if there are at least three of a kind for a given die value.
 */
    ...
    public boolean isFarkle(Hand hand) {
        // If there are valid dice left to meld, it is not a Farkle
        for (int i = 0; i < 6; i++) {
            int dieValue = hand.getCurrentHandSide(i);
            if (dieValue == 1 || dieValue == 5 || isThreeOfAKind(dieValue)) {
                return false; // Not a Farkle
            }
        }
        return true; // No valid dice to meld -> it's a Farkle
    }
