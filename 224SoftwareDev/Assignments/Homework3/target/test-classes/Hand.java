...
public int countUnmeldedDice() {
    int count = 0;
    for (int i = 0; i < currentHand.size(); i++) {
        if (currentHand.get(i).getSideUp() != 0) { // A value of 0 means it's melded
            count++;
        }
    }
    return count;
...
/**
 * Rolls only unmelded dice.
 */
public void rerollUnmeldedDice() {
    for (int i = 0; i < currentHand.size(); i++) {
        if (currentHand.get(i).getSideUp() != 0) { // Don't reroll melded dice (0 values)
            currentHand.get(i).roll();
        }
    }
    tallySides(); // Update tally after rerolling
}
}